#!/usr/bin/env python3
"""
Download the California Housing Kaggle dataset into kaggle-data/housing/raw.
...
def _copy_from_source(source: Path, dest: Path) -> None:
    for name in ("train.csv", "test.csv", "sample_submission.csv"):
        src = source / name
        if not src.exists():
            if name in ("test.csv", "sample_submission.csv"):
    ...
    print(f"[fetch-housing] Copied local files from {source} into {dest}")
...
def _download_from_kaggle(dataset: str, target_dir: Path, filename: str | None, unzip: bool) -> None:
    ...
    if is_competition:
        ...
        if filename:
            print(f"[fetch-housing] Downloading competition file {filename} from {slug}")
            api.competition_download_file(slug, filename, path=str(target_dir), quiet=False)
        else:
            print(f"[fetch-housing] Downloading competition bundle {slug} to {target_dir}")
            api.competition_download_files(slug, path=str(target_dir), quiet=False)
    else:
        if filename:
            print(f"[fetch-housing] Downloading dataset file {filename} from {dataset}")
            api.dataset_download_file(dataset, filename, path=str(target_dir), quiet=False)
        else:
            print(f"[fetch-housing] Downloading dataset {dataset} to {target_dir}")
            api.dataset_download_files(dataset, path=str(target_dir), quiet=False, unzip=unzip)

    if not unzip and not is_competition:
        print("[fetch-housing] Skipped auto-unzip per --no-unzip flag.")

    _write_stamp(target_dir, dataset)
    print(f"[fetch-housing] Dataset ready in {target_dir}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch or stage the California Housing dataset.")
    parser.add_argument(
        "--dataset",
        default="competitions/playground-series-s3e1",
    ...
    if _local_exists(target_dir) and not args.force:
        print(f"[fetch-housing] train.csv/test.csv already present in {target_dir}. Use --force to overwrite.")
        return
...
if __name__ == "__main__":
    try:
        ...
        print(f"[fetch-housing] Error: {exc}", file=sys.stderr)
        sys.exit(1)
