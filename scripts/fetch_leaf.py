#!/usr/bin/env python3
"""
Download the Leaf Classification Kaggle dataset into kaggle-data/leaf/raw.

Usage:
    python scripts/fetch_leaf.py
"""
...
def _copy_from_source(source: Path, dest: Path) -> None:
    ...
    print(f"[fetch-leaf] Copied local files from {source} into {dest}")
...
def _download_from_kaggle(dataset: str, target_dir: Path, filename: str | None, unzip: bool) -> None:
    ...
    if is_competition:
        ...
        if filename:
            print(f"[fetch-leaf] Downloading competition file {filename} from {slug}")
            api.competition_download_file(slug, filename, path=str(target_dir), quiet=False)
        else:
            print(f"[fetch-leaf] Downloading competition bundle {slug} to {target_dir}")
            api.competition_download_files(slug, path=str(target_dir), quiet=False)
    else:
        if filename:
            print(f"[fetch-leaf] Downloading dataset file {filename} from {dataset}")
            api.dataset_download_file(dataset, filename, path=str(target_dir), quiet=False)
        else:
            print(f"[fetch-leaf] Downloading dataset {dataset} to {target_dir}")
            api.dataset_download_files(dataset, path=str(target_dir), quiet=False, unzip=unzip)

    if not unzip and not is_competition:
        print("[fetch-leaf] Skipped auto-unzip per --no-unzip flag.")

    _write_stamp(target_dir, dataset)
    print(f"[fetch-leaf] Dataset ready in {target_dir}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch or stage the Leaf Classification dataset.")
    parser.add_argument(
        "--dataset",
        default="competitions/leaf-classification",
    ...
    if _local_exists(target_dir) and not args.force:
        print(f"[fetch-leaf] train.csv/test.csv already present in {target_dir}. Use --force to overwrite.")
        return
...
if __name__ == "__main__":
    try:
        ...
        print(f"[fetch-leaf] Error: {exc}", file=sys.stderr)
        sys.exit(1)
