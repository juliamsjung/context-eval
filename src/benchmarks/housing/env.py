"""California Housing Prediction environment wrapper."""
...
class HousingEnv(BaseEnv):
    """Environment wrapper around the California Housing workspace."""

    def __init__(self, workspace: Path | None = None):
        super().__init__(workspace)
        # Additional paths specific to Housing
    ...
    def _validate_required_files(self) -> None:
        """Validate that required Housing workspace files exist."""
        for required in (self.features_path, self.targets_path, self.context_path):
            if not required.exists():
                raise FileNotFoundError(
                    f"{required} not found. Run scripts/prepare_housing.py to build the workspace artifacts."
                )
