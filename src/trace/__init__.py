...
structured logging, trace events, run metadata, and run summaries.
Data from this layer must never flow into agent-visible context bundles.

Exports:
    - RunLogger: Run-scoped logger for JSONL events
    - start_run: Factory to create RunLogger instances
    - RunSummary: Aggregated run summary dataclass
...
from src.trace.run_summary import RunSummary
